...
const Emotional = ({ onMessageSent }) => {
  ...
  const quickPrompts = [
    "I cried in my dream, what does it mean?",
    "Why do my dreams feel so emotional?",
    "Does my emotional dream connect to real life?",
  ];
  ...
  const sendQuickMessage = async (text, index) => {
    ...
    try {
      await sendAiChat([userMsg], "emotional_dream", (token, fullText) => {
        setMessages((prev) => {
          const updated = [...prev];
    ...
    <div className="flex flex-col h-full overflow-hidden">
      <div className="text-[#FFFFFF] flex flex-row items-start sm:items-center mb-4">
        ...
        <p className="leading-relaxed text-[14px] sm:text-[16px]">
          Emotional dreams are filled with feelings your soul is processing.
          Tell me what you experienced, and I’ll reveal its hidden message
        </p>
      </div>
