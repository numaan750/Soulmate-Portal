...
const EnergyNumbers = ({ onMessageSent }) => {
  ...
  const quickPrompts = [
    "What do my energy numbers say about me?",
    "How can I use them for better decisions?",
    "Do they affect my daily life or relationships?",
  ];
  ...
  const sendQuickMessage = async (text, index) => {
    ...
    try {
      await sendAiChat([userMsg], "energy_number", (token, fullText) => {
        setMessages((prev) => {
          const updated = [...prev];
  ...
  const sendMessage = async () => {
    ...
    try {
      await sendAiChat(updatedMessages, "energy_number", (token, fullText) => {
        setMessages((prev) => {
          const updated = [...prev];
          updated[updated.length - 1] = {
            role: "assistant",
    ...
    <div className="flex flex-col h-full overflow-hidden">
      <div className="text-[#FFFFFF] flex flex-row items-start sm:items-center mb-4">
        ...
        <p className="leading-relaxed text-[14px] sm:text-[16px]">
          Numbers around you carry energy — let’s see what they mean for your
          journey.
        </p>
      </div>
