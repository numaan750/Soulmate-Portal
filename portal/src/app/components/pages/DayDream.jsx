...
const DayDream = ({ onMessageSent }) => {
  ...
  const quickPrompts = [
    "I daydream a lot, does it mean something?",
    "Can my daydreams show my true desires?",
    "Why do I keep imagining these things?",
  ];
  ...
  const sendQuickMessage = async (text, index) => {
    ...
    try {
      await sendAiChat([userMsg], "day_dream", (token, fullText) => {
        setMessages((prev) => {
          const updated = [...prev];
  ...
  const sendMessage = async () => {
    ...
    try {
      await sendAiChat(updatedMessages, "day_dream", (token, fullText) => {
        setMessages((prev) => {
          const updated = [...prev];
          updated[updated.length - 1] = {
            role: "assistant",
    ...
    <div className="flex flex-col h-full overflow-hidden">
      <div className="text-[#FFFFFF] flex flex-row items-start sm:items-center mb-4">
        ...
        <p className="leading-relaxed text-[14px] sm:text-[16px]">
          Daydreams reflect your hopes, desires, and future visions. Describe
          your daydream, and let’s see what it says about your path
        </p>
      </div>
      ...
      <div className="space-y-3 sm:space-y-4 flex-shrink-0">
        ...
        <div className="flex items-center bg-[#2A2D3A] rounded-full px-3 sm:px-4 py-3 sm:py-4">
          ...
          <Image
            ...
            className={`ml-3 transition w-4 h-4 sm:w-5 sm:h-5 flex-shrink-0
            ${
              input.trim()
